...
The course contest involves a multi-player contest variant of Pacman, where agents control both Pacman and ghosts in coordinated team-based strategies. 
Your team will try to carry the food on the far side of the map to their home side, while defending the food on their home side.

![](images/capture_the_flag.png)

>This is an adapted version of the [official contest page](http://ai.berkeley.edu/html).
...
There are many files in this package, most of them implementing the game itself. The **only** directory that you should work on is `team_template.py`.
It contains the only files that you will submit.
...
* `capture_agents.py`: Specification and helper methods for capture agents.
...
* `distance_calculator.py`: Computes shortest paths between all maze positions.
...
When a Pacman eats a food dot of its opponents, the food is removed and is scored when that Pacman comes back *home*. Red team scores are positive, while Blue team scores are negative.
When a Pacman is eaten by an opposing ghost, the food dots it was carrying are burst into a cloud that are left behind on the enemy field.
The Pacman then returns to its starting position (as a ghost). 
...
We will run your submissions on the UPF cluster.
To ensure that we do not overload the system, we have set the following constraints.
...
If your agent times out or otherwise throws an exception, an error message will be present in the log files, which you can download from the results page (see below).
...
```shell
$ python capture.py -r path/to/red/team/my_team.py -b path/to/blue/team/my_team.py
```
...
The `GameState` in `capture.py` should look familiar, but contains new methods like `get_red_food`, which gets a grid of food on the red side (note that the grid is the size of the board, but is only true
for cells on the red side with food). Also, note that you can list a team's indices with `get_red_team_indices`, or test membership with `is_on_red_team`.

Finally, you can access the list of noisy distance observations via `get_agent_distances`. These distances are within 6 of the truth, and the noise is chosen uniformly at random from the range [-6, 6] (e.g., if the true distance is 6, then each of {0, 1, ..., 12} is chosen with probability 1/13). You can get the likelihood of a noisy reading using `get_distance_prob`.

### Distance Calculation

To facilitate agent development, we provide code in `distance_calculator.py` to supply shortest path maze distances.
...
* Your code will be run by the following command:
    ...
    ```shell
    python capture.py --delay 0.0 -r teams/<team1>/my_team.py -b teams/<team2>/my_team.py
    ```
...
This local game was developed by [UC Berkley](http://ai.berkeley.edu/html).
